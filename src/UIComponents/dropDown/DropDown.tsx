    ...
    return (
        <div className="w-64" ref={dropdownRef}>
            <button
                ...
                className="w-full bg-slate-50 justify-between focus:outline-none text-slate-400 py-2 px-3 pr-8 leading-tight flex items-center transition duration-150 ease-in-out"
            >
                <span>{value?.value === 'Freshers' ? 'Freshers' : value?.value ? value?.label : "Select experience"}</span>
                <svg
                    className={`fill-current h-4 w-4 transform z-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
